...
function Neo4jD3(_selector, _options) {
    ...
    function appendOutlineToRelationship(r) {
        //console.log(d.properties['built_capacity']+"#"+d.properties['utilization']);
        ...
        return r.append('path')
                ...
                .attr('stroke', function(d) {
                                  var stroke_color;
                                  if ((d.properties['built_capacity']*0.4)-d.properties['utilization']>0) {
                                    stroke_color = "#20c997";
                                  }else{
                                    stroke_color = "#dc3545";
                                  }
                ...
                .attr('stroke-width', function(d){return Math.log(d.properties['built_capacity']/10000)*3 + 2;});
    }
...
//            color = options.colors[Math.min(numClasses, options.colors.length - 1)];
    ...
    function checkAlert(val,chk) {
        
        var node = svgNodes.selectAll('.node');
        //console.log(node);
        //console.log(JSON.stringify(node._groups["0"]));
        //console.log(node._groups[0][50].__data__);
        //node._groups[0][50].style.visibility = "hidden";
        for(var i=0; i<node._groups[0].length; i++){
          //console.log(node._groups[0][i].__data__.id);
          //console.log(node._groups[0][i].__data__.properties.partOfBB);
          if (node._groups[0][i].__data__.properties.partOfBB.indexOf(val)!= -1) {
            if (chk==1) {
              node._groups[0][i].style.visibility = "visible";
              console.log("visible"+val);
            }
            else {
              node._groups[0][i].style.visibility = "hidden"; 
              console.log("hidden"+val);
            }
          };
          /*var dd = node._parents[0].childNodes[i].__data__;
          console.log(dd);*/
          //console.log(node._parents[0].childNodes[i].__data__.properties.partOfBB);
        }
    }

    function checkBlert(val,chk) {
        
        var relationship = svgRelationships.selectAll('.relationship');
        ...
        for(var i=0; i<relationship._groups[0].length; i++){
          //console.log(relationship._groups[0][i].__data__.id);
          //console.log(relationship._groups[0][i].__data__.properties.partOfBB);
          if (relationship._groups[0][i].__data__.properties.partOfBB.indexOf(val)!= -1) {
            if (chk == 1) {
              relationship._groups[0][i].style.visibility = "visible";
            }
            else {
              relationship._groups[0][i].style.visibility = "hidden"; 
            }
          };
          /*var dd = node._parents[0].childNodes[i].__data__;
          console.log(dd);*/
    ...
    function checkClert(bigArr) {
        ...
        for(var i=0; i<node._groups[0].length; i++){
          ...
          for(var j=0; j<bigArr.length; j++){
            ...
            if (node._groups[0][i].__data__.properties.partOfBB.indexOf(val)!= -1) {
              //console.log("node: "+node._groups[0][i].__data__.properties.partOfBB);
              //console.log("val: "+val);
              //console.log("chk: "+chk);
              console.log("match: "+node._groups[0][i].__data__.properties.id+"#"+node._groups[0][i].__data__.properties.partOfBB+"#"+val);
              //if (chk==1) {
                //console.log("visible");
                node._groups[0][i].style.visibility = "visible";
                //console.log("visible"+val);
        ...
        for(var i=0; i<relationship._groups[0].length; i++){
          ...
          for(var j=0; j<bigArr.length; j++){
            ...
            if (relationship._groups[0][i].__data__.properties.partOfBB.indexOf(val)!= -1) {
              ...
              //console.log("val: "+val);
              //console.log("chk: "+chk);
              console.log("match: "+relationship._groups[0][i].__data__.properties.id+"#"+relationship._groups[0][i].__data__.properties.partOfBB+"#"+val);
              //if (chk==1) {
                //console.log("visible");
                relationship._groups[0][i].style.visibility = "visible";
                //console.log("visible"+val);
...
*/
    ...
    function updateInfo(d) {
        ...
        //alert(d.properties['built_capacity']);
    }
...
//        smoothTransform(svgTranslate, svgScale);
    ...
    return {
        ...
        checkBlert:checkBlert,
        checkClert:checkClert
    };
}
